...
// 2. Create a function to check if we can move the element (tile)
const canIMove = (tile) => {
// find the empty tile
...
// get the row index of the tile we clicked on
 ...
 // check all possibilities if a tile can move
  return (emptyTileRow === tileRow && emptyTileColumn === tileColumn - 1) ||
         (emptyTileRow === tileRow && emptyTileColumn === tileColumn + 1) ||
         (emptyTileColumn === tileColumn && emptyTileRow === tileRow - 1) ||
         (emptyTileColumn === tileColumn && emptyTileRow === tileRow + 1)
}
...
// and run the function directly above;
...
// 5. For each tile we need to run the above functions
tiles.forEach( (tile) => {
