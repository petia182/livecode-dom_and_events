...
const checkIfPlayerWins = () => {
  // get the order of the tiles (which will be checked after each click) and convert it into an array
  // each element in the array represents the innerText of the tile
  const tileOrder = Array.from(tiles).map(tile => tile.innerText);

  // convert the array to string (using .join() )
  // compare the tileOrder with this manually sorted array to check if the tiles are ordered in the correct order
...
// 5. For each tile ON CLICK, we need to call the above functions
tiles.forEach( (tile) => {
